...
class LuxtronikCoordinator(DataUpdateCoordinator[LuxtronikCoordinatorData]):
    ...
    def get_device_entity_title(self, key: str, platform: Platform | str) -> str:
        ...
        if "entity" in self.__content_locale__.keys():
            entity_node: dict = cast(dict, self.__content_locale__.get("entity"))
            if platform in entity_node:
                platform_node: dict = cast(dict, entity_node.get(str(platform)))
                if key in platform_node.keys():
                    key_node: dict = cast(dict, platform_node.get(key))
                    if "name" in key_node.keys():
                        return str(key_node.get("name"))
        LOGGER.warning(
